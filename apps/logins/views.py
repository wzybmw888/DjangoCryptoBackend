...
class SmsSerializer(serializers.Serializer):
    ...
    def validate_mobile(self, mobile):
        ...
        if self.context['smstype'] == "restpass" or self.context['smstype'] == "login":  # 重置密码/用户登录
            # 该手机号是否已注册
            if not Users.objects.filter(username=mobile, mobile=mobile, identity__contains="2",
                                        is_active=True).count():
                raise serializers.ValidationError("没有找到该用户", 400)

        if self.context['smstype'] == "wxbind":  # 微信绑定
            # 该手机号是否已注册
            if not Users.objects.filter(username=mobile, mobile=mobile, identity__contains="2", is_active=True,
...
class SendSmsCodeView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if smstype == "login" or smstype == "restpass" or smstype == "wxbind" or smstype == "register":
            ...
            # 存储一个标记，表示此手机号已发送过短信，标记有效期为60s
            redis_conn.setex('send_flag_%s' % mobile, 60, 1)
            return SuccessResponse(data=code, msg="success")
            # 无短信发送-----------结束
        else:
...
class APPMobileSMSLoginView(APIView):
    ...
    def post(self, request):
        ...
        if not send_flag:  # 如果取不到标记，则说明验证码过期
            ...
            # 开始登录
            user = Users.objects.filter(username=mobile + "app", mobile=mobile, identity__contains="1",
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not send_flag:  # 如果取不到标记，则说明验证码过期
            ...
            if str(send_flag.decode()) != str(code):
                return ErrorResponse(msg="验证码错误")
            # 开始更换手机号
            user = Users.objects.filter(username=mobile, mobile=mobile, identity__contains="2",
                                        is_active=True).first()
            if not user:
                return ErrorResponse(msg="用户不存在")
            # 重置密码
...
class RegisterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not re.match(r"^.*(?=.{6,})(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*? ]).*$", password):
            return ErrorResponse(msg="密码最少6位，包括至少1个大写字母，1个小写字母，1个数字，1个特殊字符")

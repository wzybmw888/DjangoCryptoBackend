...
from config import WX_XCX_APPID, WX_XCX_APPSECRET, WX_GZH_APPID, WX_GZH_APPSECRET, WX_GZH_TOKEN, WX_GZPT_APPSECRET, \
    WX_GZPT_APPID, TT_XCX_APPID, TT_XCX_APPSECRET, DOMAIN_HOST
import requests
import base64
import json
        ...
        try:
            ...
            with transaction.atomic():
                try:
                    ...
                    user.save()
                    OAuthWXUser.objects.create(user=user, session_key=session_key, xcx_openid=openid,
                                               avatarUrl=avatar_url, sex=gender,
                                               mobilePhoneNumber=pResult['phoneNumber'], nick=nickname)
                    # if inviter:#如果存在邀请码
                    #     integral = FenXiaoManage.objects.filter(type=1,status=True).values_list('content',flat=True).first()
                    #     if integral:#如果推广积分活动还存在
                    #         Users.objects.filter(id=inviter).update(integral=F('integral')+int(integral))
        ...
        if 'errcode' in json_data:  # 如果获取失败返回失败信息
            if json_data['errcode'] == 40001:
                ...
                if 'errcode' in json_data2 and json_data2['errcode'] != 0:
                    logger.error(
                        "微信小程序发送消息服务错误，用户openid:%s，template_id:%s，form_id:%s，data:%s，微信返回错误信息：%s" % (
...
class WeChatGZHBindAPIView(APIView):
    ...
    def post(self, request):
        ...
        user = Users.objects.filter(is_active=True, username=mobile + "app", identity__contains="1",
...
class WeChatGZHH5LoginAPIView(APIView):
    ...
    def get(self, request):
        ...
        if not user:  # 如果不存在则提示绑定用户关系
            ...
            user = Users.objects.create(username=openid, password=uuid.uuid4(), is_staff=False, is_active=True,
                                        nickname=nickname, name=nickname, avatar=avatar, gender=sex)
            OAuthWXUser.objects.create(user=user, gzh_openid=openid, avatarUrl=avatar, nick=nickname, sex=sex,
                                       gzh_access_token=access_token)
        ...
        try:
            ...
            with transaction.atomic():
                try:
                    ...
                    user.save()
                    OAuthWXUser.objects.create(user=user, session_key=session_key, xcx_openid=openid,
                                               avatarUrl=avatar_url, sex=gender, nick=nickname)
                except Exception as e:
                    transaction.savepoint_rollback(savepoint)
                    return ErrorResponse(msg=str(e))
                # 清除保存点
